...
export default class CactusTile extends Tile {
  ...
  protected hurtMob(
    ...
    // TODO: Finish implementing CactusTile.hurtMob.
    // level.add(new SmashParticle(x * 16 + 8, y * 16 + 8));
    // level.add(new TextParticle("" + dmg, x * 16 + 8, y * 16 + 8, PALETTE.get(-1, 500, 500, 500)));
    // if (damage >= 10) {
    // 	const count = Random.nextInt(2) + 1;
    // 	for (let i = 0; i < count; i++) {
    // 		level.add(new ItemEntity(new ResourceItem(Resources.cactusFlower), x * 16 + Random.nextInt(10) + 3, y * 16 + Random.nextInt(10) + 3));
    // 	}
