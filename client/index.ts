...
import { PALETTE } from './system/display/palette';
import Font from './system/display/Font';
import { ButtonUIElement, LabelUIElement, ProgressMeterUIElement, UIElement } from './system/ui/index';
...
const onMouseMove = (e: MouseEventProxy) => {
...
const onMouseDown = (e: MouseEventProxy) => {
...
const onMouseUp = (e: MouseEventProxy) => {
  if (e.button === 0) {
    if (isPlayerSelected) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  image = new Image(await window.api.gfx.getTiles())
  tileset = new system.display.TileSet(image, 8, 8);
  font = new Font(tileset);

  mouseCursor = new system.display.Sprite(tileset, 0, 29, [-1, -1, -1, 555], 1);
  mouseCursor.moveTo(system.display.getWidth() / 2, system.display.getHeight() / 2);

  player = new system.display.Sprite(tileset, 0, 14, [-1, 100, 220, 532]);
  player.moveTo(50, 50);
  sprites.push(player);
